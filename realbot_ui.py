...
def main(dataset : ModelParams, iteration : int, opt) -> None:
    ...
    # scale+center alignment: COLMAP units -> meters, placed in front of robot
    # generated by: python scripts/compute_alignment.py --real_size 0.05 --target_pos 0.5 0.0 0.05
    _w2b_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             dataset.model_path, "world2base.npy")
    world2base = np.load(_w2b_path) if os.path.exists(_w2b_path) else np.eye(4)

    # bbox in robot base frame (meters)
    x_min = 0.2
    x_max = 0.8
    y_min = -0.3
    y_max = 0.3
    ...
    sparse_dir = next((os.path.join(dataset.source_path, p) for p in ["colmap/sparse/0", "sparse/0"]
                       if os.path.exists(os.path.join(dataset.source_path, p))), os.path.join(dataset.source_path, "sparse/0"))
    pcd_tsdf = o3d.io.read_point_cloud(os.path.join(sparse_dir, "points3D.ply"))
